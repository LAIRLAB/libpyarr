...
void featpyramid(IN const mxArray * input, IN const int sbin, IN const int interval, OUT vector<mxArray*> &feat, OUT vector<real> &scale)
{
    ...
    if (0) {
        printf("max_scale = %d\n", max_scale);
        printf("input dims is still (%d, %d, %d)\n", 
               input->Dims[0],
               input->Dims[1],
               input->Dims[2]);
    }
    ...
    scale.resize(max_scale + interval, 0.0);
	...
	feat[i+interval] = features(scaled, mxsbin);
        if (0) {
            printf("features %d has shape (%d, %d, %d)\n",
                   i+interval,
                   feat[i+interval]->Dims[0],
                   feat[i+interval]->Dims[1],
                   feat[i+interval]->Dims[2]); 
        }
	mxFree(&mxsbin);
