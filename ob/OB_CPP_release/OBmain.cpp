...
int loadModelFiles(string dir, vector<CModel>& models, string mvalue) {
    ...
    }/* ocean sky beach mountain jersey seashore building streetlight */
...
mxArray * loadImg(const char *filename)
{
    ...
    if (cinfo.num_components != 3) {
        std::cerr << "only accept 3-channel image" << endl;
        exit(-1);
    } 
...
int extractOBFeature(mxArray * input, vector<CModel> &modelList, bool saveMap, string fileName, vector<float> & outputFeature, vector<vector<mxArray *> > & responsemaps, int numComponents, int numLevels)
{
    ...
    foreach (it, feat) {
        if (0) {
            if (*it != NULL) {
                printf("feature %d has ndim %d, dims %d, %d, %d\n", 
                       counter2, (*it)->NDim, (*it)->Dims[0], (*it)->Dims[1], (*it)->Dims[2]);
            }
            else printf("feature %d is null\n", counter2);
        }
	counter2++;
    }
	...
	responsemaps.push_back(responsemap);
...
int main(int argc, char *argv[]){
    ...
    vector<vector<mxArray *> > responsemap;	//outputs of detect() are stored here
    ...
    for (int i = 0; i < fileNames.size(); i++) 
    {
        ...
        try {
            ...
            if (0) 
                printf("resized has size (%d, %d)\n",
                       input_resized->Dims[0], input_resized->Dims[1]);
...
class ob_runner {
    ...
    ob_runner(vector<string> l) {
	foreach (it, l) {
	    models.push_back(CModel(*it));
	}
    }
